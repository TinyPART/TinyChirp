...
static io_t raw_audio_buf[RESAMPLE_RATE];
void unit_test(void);
int sd_storage_test(void);
void preprocess_test(void);
int main(void)
{
    ...
    //     for (int i = 0; i < RESAMPLE_RATE; i++)
    //     {
    //         raw_audio_buf[i] = 10.0 / RESAMPLE_RATE;
    //     }
    ...
    //     for (int i = 0; i < 3; i++)
    //     {
    //         find_min_max(raw_audio_buf, RESAMPLE_RATE, &scaler);
    //         apply_min_max_scaler(raw_audio_buf, RESAMPLE_RATE, raw_audio_buf, &scaler);
    //         apply_cascaded_sos(sos_filters, NUM_OF_SOS, raw_audio_buf, raw_audio_buf, RESAMPLE_RATE);
    //         io_t energy = calculate_energy(raw_audio_buf, RESAMPLE_RATE);
    //         (void) energy;
    //     }
    //     end = xtimer_now_usec();
    //     printf("Compute Latency usec: %ld\n", (long int)(end - start));
...
void preprocess_test(void) {
    ...
    int downsample_len;
    downsample(raw_audio_buf, sizeof(raw_audio_buf) / sizeof(io_t),
               raw_audio_buf, &downsample_len, SAMPLE_RATE / RESAMPLE_RATE);
}
...
void unit_test(void)
{
    ...
    sos_coeff_t highpass_coeff[9][6] = {
